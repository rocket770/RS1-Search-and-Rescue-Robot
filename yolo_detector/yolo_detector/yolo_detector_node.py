...
# class TwoDTo3DPose:
#     """
#     Simple 2D->3D projector.
#     - Subscribes to /camera/camera_info (intrinsics K)
#     - Subscribes to /camera/depth/image (depth)
#     - Uses TF to transform camera-frame point -> target_frame
#     - get_xyz((x1,y1,x2,y2), stamp=...) returns (x,y,z) for the bbox bottom-centre.
#       If called with a non-bbox (e.g., string), returns None.
#     """
...
#         """
#         depth_scale: 1.0 if depth is already meters (e.g., 32FC1),
#                      0.001 if depth is millimetres (e.g., 16UC1).
#         """
#         # config
...
#         # internal node (so you don't modify your detector node)
...
#         # subscriptions (sensor-data QoS typical for cameras)
#         self._sub_caminfo = self._node.create_subscription(
#             CameraInfo, 
...
#                 img = img.astype(np.float32) * self.depth_scale
#             self._depth = img
...
#         """
#         bbox: tuple/list (x1,y1,x2,y2) in pixel coords.
#         Returns (x,y,z) in target_frame for the bbox **bottom-centre**, or None.
#         """
...
#         # 2) depth (meters)
#         z = float(self._depth[v, u])
#         if not np.isfinite(z) or z <= 0.0:
#             return None

#         # 3) unproject: X_cam = z * K^{-1} * [u, v, 1]^T
#         pix = np.array([u, v, 1.0], dtype=np.float64)
#         Xc = self._Kinv @ pix
#         Xc *= z

#         # 4) TF to target_frame
#         pt = PointStamped()
#         pt.header.frame_id = self.camera_frame
#         pt.header.stamp = stamp if stamp is not None else (self._depth_stamp or self._node.get_clock().now().to_msg())
#         pt.point.x, pt.point.y, pt.point.z = float(Xc[0]), float(Xc[1]), float(Xc[2])

#         try:
#             out = self._tf_buffer.transform(pt, self.target_frame, timeout=Duration(seconds=0.1))
...
#             subscribe_to_depth=False  # ADD THIS - We'll sync manually
#         )
...
#             [self.sub_rgb, self.sub_depth],  # CHANGED: added depth
#             queue_size=10, 
#             slop=0.05  # CHANGED: tighter tolerance (50ms)
#         )
...
#                         f"found box but could not compute 3D point for {label_name} (no depth/intrinsics/TF yet?)"
#                     )
#                     continue
